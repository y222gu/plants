...
Usage:
    python train/train_unet_5c.py --strategy A
...
def main():
    ...
    parser.add_argument("--data-dir", type=str, default=None,
                        help="Data directory containing image/ and annotation/ subdirectories. "
                             "Defaults to project data/ folder.")
    ...
    parser.add_argument("--strategy", default="A", choices=["A", "B", "C"])
    ...
    if args.no_val:
        ...
        print(f"\n--no-val: Training on ALL {len(registry.samples)} samples, "
              f"no validation, {args.epochs} epochs")
        split = {"train": registry.samples, "val": [], "test": []}
    elif args.data_dir:
        # Custom data dir: use simple split logic
        # Force non-Tomato samples (Rice, Sorghum, Millet) into training set,
        # split only Tomato by experiment for train/val/test.
        from collections import defaultdict
        import random
        rng = random.Random(args.seed)

        forced_train = [s for s in registry.samples if s.species != "Tomato"]
        tomato = [s for s in registry.samples if s.species == "Tomato"]

        # Group Tomato by experiment
        ...
        split = {
            "train": forced_train + [s for k in train_keys for s in exp_groups[k]],
            "val": [s for k in val_keys for s in exp_groups[k]],
            "test": [s for k in test_keys for s in exp_groups[k]],
        }
        print(f"\nForced {len(forced_train)} non-Tomato samples into training set")
    else:
    ...
    # Callbacks
    run_name = f"{args.arch}_{args.encoder}_{args.strategy}_multilabel_c5_full"
    if args.species:
